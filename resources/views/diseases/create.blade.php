    ...
    <div id="formContainer" class="hidden bg-white shadow-md rounded-lg p-4">
      <form method="POST" action="{{ route('diseases.store') }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="block">
            ...
            <textarea id="name" name="name" class="form-textarea mt-2 block w-full rounded-md border-gray-400 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-200 focus:ring-opacity-75" rows="3">{{old('name')}}</textarea>
            @error('name')
