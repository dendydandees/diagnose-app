              ...
              @if (Route::has('login'))
                <div class="hidden sm:block sm:ml-6">
                  <div class="flex space-x-4">
                    @auth
                      <a href="{{ route('dashboard') }}" class="{{ Route::is('dashboard') ? 'bg-gray-900 text-white' : 'text-gray-300' }} px-3 py-2 rounded-md text-sm font-medium hover:bg-gray-700 hover:text-white">
                          {{ __('Dashboard') }}
                      </a>
                    @else
        ...
        @if (Route::has('login'))
          <div :class="{'block' : open, 'hidden' : !open}" class="transition-all duration-500 ease-in-out sm:hidden" id="mobile-menu">
            <div class="pt-2 pb-3 space-y-1">
              @auth
                <a href="{{ route('dashboard') }}" class="{{ Route::is('dashboard') ? 'bg-gray-900 text-white' : 'text-gray-300' }} hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{ __('Dashboard') }}</a>
              @else
                <a href="{{ route('login') }}" class="{{ Route::is('login') ? 'bg-gray-900 text-white' : 'text-gray-300' }} hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{ __('Sign In') }}</a>
                @if (Route::has('register'))
                  <a href="{{ route('register') }}" class="{{ Route::is('register') ? 'bg-gray-900 text-white' : 'text-gray-300' }} hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium">{{ __('Sign up') }}</a>
                @endif
              @endauth
            </div>
          </div>
